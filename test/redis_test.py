...
SERVER_NORMAL_ERR = (
    "msg /home/netease/Projects/redis/include/connect.h:82 try_fill_buffer: EOF"
)
...
try:
    ...
    server_proc = subprocess.Popen(
        ["../build/src/main/server"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )

    client_commands = [
        ["../build/src/main/client", "set", "key1", "value1"],
        ["../build/src/main/client", "get", "key1"],
        ["../build/src/main/client", "set", "key2", "value2"],
        ["../build/src/main/client", "get", "key2"],
        ["../build/src/main/client", "keys"],
        ["../build/src/main/client", "del", "key1"],
        ["../build/src/main/client", "del", "key2"],
    ]
    ...
    for cmd, ans in zip(client_commands, commands_answer):
        ...
        # 下面要读取服务端的输出
        # 因为redis的join的嘛, 所以不能干等着, 得使用IO复用的方式
        while True:
