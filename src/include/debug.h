...
std::string block2string(const char* block) {
    ...
    for (int i = 0; i < cmd_num; ++i) {
        ...
        std::string_view cmd{reinterpret_cast<const char*>(block + cur + 4), cmd_len};

        ss << "\033[93m" << std::setw(2) << cmd_len << "\033[0m \"" << cmd << "\"\033[0m"
           << (i == cmd_num - 1 ? " " : ", ");
...
std::ostream& operator<<(
    typename std::enable_if<std::is_enum<T>::value, std::ostream>::type& ostream, const T& e) {
    return ostream << "\033[93m" << typeid(e).name() << "\033[0m::"
                   << "\033[36m" << static_cast<typename std::underlying_type<T>::type>(e)
                   << "\033[0m";
}