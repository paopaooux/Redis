...
public:
    ...
    public:
        ...
        iterator& operator++() {
            ptr_ = container_of(avl::offset(&ptr_->avl_node, +1), ZNode, avl_node);
            step_++;
            return *this;
        }
    ...
    ZNodeCollection(ZNode* data, int64_t length) : data_{data}, length_{length} {}
    ...
    friend std::ostream& operator<<(std::ostream& os, const ZNodeCollection& znode_colloction) {
        return os << class2str("ZNodeCollection", "data_", znode_colloction.data_, "length_",
...
private:
    ...
    void tree_add(ZNode* node) {
        ...
        while (true) {
            avl::AVLNode** from = zless(&node->avl_node, cur) ? &cur->left : &cur->right;
            if (!*from) {
                *from = &node->avl_node;
...
public:
    ...
    ZNodeCollection query(double score, const std::string& name, int64_t offset, int64_t limit) {
        ...
        return found ? ZNodeCollection(container_of(found, ZNode, avl_node), limit) :
